#!/usr/bin/env python3
"""(Re)write dongle-pro-power.kicad_pro from the 1s4l project template.
...
def write_pro():
    ...
    p['meta']['filename'] = 'dongle-pro-power.kicad_pro'
    p['schematic']['top_level_sheets'] = [{
        'filename': 'dongle-pro-power.kicad_sch', 'name': 'dongle-pro-power',
    ...
    p['net_settings']['netclass_patterns'] = (
        ...
        [{'netclass': 'PWR', 'pattern': pat} for pat in
         # real power rails only; gate/enable/sense logic stays Default so it
         # can route thin through the congested divider column
         ['+5V', '+3V3', '+1V1', '+1V2', 'HUB_3V3', 'HUB_1V2', 'HUB_SW',
          'HUB_AV12', 'VREG_LX', 'TGT_VBUS', 'PWR_VBUS', 'SRC20', 'SRC20_MID',
          'VSAFE_SRC', 'VSAFE_MID', '+5V_BUCK', 'HOST_VBUS', 'BUCK_PH']])
    ...
    json.dump(p, open(os.path.join(HERE, 'dongle-pro-power.kicad_pro'), 'w'), indent=2)

if __name__ == '__main__':
    write_pro()
    print('dongle-pro-power.kicad_pro written')
