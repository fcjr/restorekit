...
/// Written as a column-0 raw string on purpose: a `\`-continued literal drops
/// the newline *and* the next line's leading whitespace, which flattened every
/// step and bullet against the left margin. Prose is hard-wrapped so terminals
/// don't reflow it into the indentation.
const MANUAL_DFU_INSTRUCTIONS: &str = r#"This host cannot trigger DFU mode electronically (requires an Apple Silicon
Mac running macOS). Put the target Mac into DFU mode manually:

  1. Connect the target to this host with a USB-C cable using the target's
     DFU port:
  ...
  3. Apple silicon laptop: hold the power button, then while holding it,
     connect power and keep holding ~10s.
     Desktop: unplug power 10s, then press and hold the power button while
     reconnecting power.
  4. Release. The screen stays black in DFU mode. Re-run this command to
     detect it."#;
